...
export default function PageCompare({ onOpenCart }) {
  ...
  function toggle(p) {
    let idx = -1;
    for (let i = 0; i < selected.length; i++) {
      if (selected[i].id === p.id) { idx = i; break; }
    }
    if (idx > -1) {
      const next = [];
      for (let i = 0; i < selected.length; i++) {
    ...
    return (
      <table className="compareTable">
        ...
        <tbody>
          ...
          <tr><td>Price</td><td>₦{a.price.toLocaleString("en-NG")}</td><td>₦{b.price.toLocaleString("en-NG")}</td></tr>
        </tbody>
      </table>
    );
  }, [selected]);
    ...
    <h2 className="pageTitle1">Compare</h2>
      ...
      <div className="comparePickGrid">
        {filtered.map(p => {
          ...
          return (
            <article className={sel ? "productCard pickSelected" : "productCard"} key={p.id}>
              ...
              <div className="productMain">
                ...
                <div className="productActions">
                  ...
                  <button
                    ...
                    onClick={() => Cart.add({ id: p.id, title: p.title, price: p.price, image: p.image }, 1)}
                  >
                    Quick add
                  </button>
                </div>
      ...
      {selected.length === 2 && (
        <div className="compareActions">
          <button className="buttonPrimary" onClick={addBoth}>Add both to cart</button>
        </div>
      )}
